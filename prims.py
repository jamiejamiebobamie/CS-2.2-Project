...
def primMST(graph,edges):
    ...
    if (not len(test_set_from_vert)
        == len(test_set_to_vert)
        == len(graph.vertices)):
            return """For this algorithm to work,
    ...
    while len(MST) < graph.numberOfVertices-1:
        ...
        for i, edge in enumerate(edges):
            if (float(edge[2]) < float(minEdge[2]) and
                edge[1] not in visited and
                edge[0] in visited and edge[0] != edge[1]):
                    minEdge = edge
                    minEdge_index = i
